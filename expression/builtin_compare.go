...
func (c *coalesceFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	bf, err := newBaseBuiltinFuncWithTp(ctx, args, retEvalTp, fieldEvalTps...)
	if err != nil {
		return nil, err
	}
...
func GetCmpFunction(ctx sessionctx.Context, lhs, rhs Expression) (CompareFunc, error) {
	switch GetAccurateCmpType(lhs, rhs) {
	case types.ETInt:
		return CompareInt, nil
	case types.ETReal:
		return CompareReal, nil
	case types.ETDecimal:
		return CompareDecimal, nil
	case types.ETString:
		_, dstCollation, dstFlen, _ := DeriveCollationFromExprs(ctx, lhs, rhs)
		return genCompareString(dstCollation, dstFlen), nil
	case types.ETDuration:
		return CompareDuration, nil
	case types.ETDatetime, types.ETTimestamp:
...
func (c *compareFunctionClass) generateCmpSigs(ctx sessionctx.Context, args []Expression, tp types.EvalType) (sig builtinFunc, err error) {
	bf, err := newBaseBuiltinFuncWithTp(ctx, args, types.ETInt, tp, tp)
	if err != nil {
		return nil, err
	}
	if tp == types.ETJson {
