...
func (c *isTrueOrFalseFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	bf, err := newBaseBuiltinFuncWithTp(ctx, args, types.ETInt, argTp)
	if err != nil {
		return nil, err
	}
	bf.tp.Flen = 1
...
func (c *bitNegFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	bf, err := newBaseBuiltinFuncWithTp(ctx, args, types.ETInt, types.ETInt)
	if err != nil {
		return nil, err
	}
...
func (c *unaryMinusFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (sig builtinFunc, err error) {
	...
	case types.ETInt:
		if intOverflow {
			bf, err = newBaseBuiltinFuncWithTp(ctx, args, types.ETDecimal, types.ETDecimal)
			if err != nil {
				return nil, err
			}
