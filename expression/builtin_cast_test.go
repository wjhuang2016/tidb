...
func (s *testEvaluatorSuite) TestCastFuncSig(c *C) {
	...
	for i, t := range castToDecCases {
		...
		baseBf, err := newBaseBuiltinFunc(ctx, args)
		c.Check(err, IsNil)
		decFunc := newBaseBuiltinCastFunc(baseBf, false)
	...
	for i, t := range castToIntCases {
		...
		baseBf, err := newBaseBuiltinFunc(ctx, args)
		c.Check(err, IsNil)
		intFunc := newBaseBuiltinCastFunc(baseBf, false)
		switch i {
		case 0:
	...
	for i, t := range castToRealCases {
		...
		baseBf, err := newBaseBuiltinFunc(ctx, args)
		c.Check(err, IsNil)
		realFunc := newBaseBuiltinCastFunc(baseBf, false)
		switch i {
		case 0:
	...
	baseBf, err := newBaseBuiltinFunc(ctx, args)
	c.Check(err, IsNil)
	sig = &builtinCastStringAsIntSig{newBaseBuiltinCastFunc(baseBf, false)}
...
func (s *testEvaluatorSuite) TestCastJSONAsDecimalSig(c *C) {
	...
	baseBf, err := newBaseBuiltinFunc(ctx, []Expression{col})
	c.Check(err, IsNil)
	decFunc := newBaseBuiltinCastFunc(baseBf, false)
	decFunc.tp = types.NewFieldType(mysql.TypeNewDecimal)
	decFunc.tp.Flen = 60
